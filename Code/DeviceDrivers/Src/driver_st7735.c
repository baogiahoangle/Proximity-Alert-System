...
st7735_status_t st7735_write_cmd(st7735_t *me, uint8_t cmd)
{
  ...
  gpio_write_pin(me->port, me->pin_a0, GPIO_STATE_RESET); // A0 = 0 for command
  gpio_write_pin(me->port, me->pin_cs, GPIO_STATE_RESET); // CS = 0, select display

  if (HAL_SPI_Transmit(me->hspi, &cmd, 1, ST7735_SPI_TIMEOUT) != HAL_OK)
  {
    gpio_write_pin(me->port, me->pin_cs, GPIO_STATE_SET); // CS = 1, deselect display
...
st7735_status_t st7735_write_data(st7735_t *me, uint8_t data)
{
  ...
  gpio_write_pin(me->port, me->pin_a0, GPIO_STATE_SET);   // A0 = 1 for data mode
  gpio_write_pin(me->port, me->pin_cs, GPIO_STATE_RESET); // CS = 0, select display
...
st7735_status_t st7735_init(st7735_t *me)
{
  ...
  gpio_write_pin(me->port, me->pin_reset, GPIO_STATE_RESET);                      // Pull reset pin low
  HAL_Delay(ST7735_RESET_DELAY);                                                   // Hold reset for 20ms
  gpio_write_pin(me->port, me->pin_reset, GPIO_STATE_SET);                        // Release reset
